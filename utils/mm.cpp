...
// std::vector<int> MemoryController::searchBitmapInNumMode(int num) {
//     // 顺序遍历每个位
//     std::vector<int> results;
//     int count = num;
//     bool flag = false;
//     // TODO: 这里后期可以该为多线程并行计算
//     for (size_t i = 0; i < ROUTE_SEGMENT_NUM/8; i++)
//     {
//         for (size_t j = 0; j < 8; j++)  // 分别遍历每个bit
//         {
//             if (((this->hostBitmap[i]>>(7-j)) & 0x1) == 0)  // 当前位为0——表示空闲
//             {
//                 this->hostBitmap[i] = this->hostBitmap[i] | (0x1 << 7-j);  // 在这里直接设置bitmap的对应位
//                 results.push_back(i*8+j);
//                 --count;
//                 if (count == 0)
//                 {
//                     flag = true;
...
//     if (count != 0)
//     {
//         std::cerr << "Route heap space is not enough." << std::endl;
//         exit(-1);
//     }
//     return results;
// }